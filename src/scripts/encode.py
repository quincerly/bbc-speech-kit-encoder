...
def main_cli():
    ...
    if len(sys.argv) < 3:
        print("Usage: python3 encode.py input.wav output.ssd")
        sys.exit(1)

    input_wav  = sys.argv[1]
    output_ssd = sys.argv[2]

    # Load WAV and downsample to 8kHz
    ...
    # Analyse
    preemph=0.30; bwe=0.985; voiced_thresh=0.30; min_f0=75; max_f0=300; max_gap=2
    ...
    nv = sum(1 for f in frames if f['voiced'])
    print(f"Voiced: {nv}/{total_frames}")

    # Encode to TMS5220 bitstream
    ...
    # Build tokenised BBC BASIC player program
    ...
    # Build DFS disk image — 800 sectors = 204800 bytes (matches original working images)
    ...
    print(f"Written: {output_ssd} ({len(disk)} bytes)")
